...
const PlaceOrderScreen = ({ history }) => {
  ...
  const { cartItems, shippingAddress: { address, postalCode, city, country } } = cart
  ...
  return (
    errorMessage ? <Message variant='danger'>{errorMessage}</Message> :
      <div>
        ...
        {
          cartItems.length === 0 ? <Message>Your cart is empty. <Link to='/'>Back to homepage</Link></Message> : (<Row>
            <Col md={8}>
